...
- `validate_usage({ code, css? })` — deterministic snippet validation:
  ...
  hallucinated primitives, raw color/size literals, space-tokens-as-sizes,
  cross-component hook borrowing, pseudo-class state selectors, Tailwind
  classes. Returns `{ valid, violations: [{ rule, message, fix }] }` with
  wrong→right messages sourced from the live negative-rule catalog.
