...
| FB-16 | First-pass gauntlet metric counts every local dev-loop run against a release (all-runs-at-sha semantics), so using the gauntlet iteratively poisons the number; and requiredGauntletSteps evaluated INSIDE the gauntlet reads the previous run's line — deadlocks after any fail-fast run | Fleet rollup switches to first-run-per-sha semantics; requiredGauntletSteps gains a current-run input (or stays fleet-side-only, as documented) | fleet |
| FB-17 | Marketing numbers were hand-maintained on three surfaces (README/site/deck) and drifted twice — external reviewer caught 662-vs-~1,300; measured truth was 1,013 | Generate a stats.json from the gauntlet/metrics (test totals, package count, registry counts) and have README badges, site Stats, and the deck build read from it — same compiled-not-authored rule as everything else | validate + site |
